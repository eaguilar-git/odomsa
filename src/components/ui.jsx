...
export function Badge({ children, color = 'teal' }) {
  ...
  return (
    <span style={{ display:'inline-flex', alignItems:'center', padding:'0.2rem 0.6rem', borderRadius:'99px', fontSize:'0.75rem', fontWeight:600, background:c.bg, color:c.text }}>
      {children}
    </span>
  )
}

export function StatusBar({ message, url, lastSync, onRefresh, onTogglePause, paused, catalogDirty }) {
  return (
    <div style={statusBar}>
      ...
      {catalogDirty && <Badge color="amber">Borrador catalogo</Badge>}
      <div style={{ marginLeft:'auto', display:'flex', gap:'0.5rem' }}>
        <Btn size="sm" variant="ghost" onClick={onRefresh}>&#8635; Actualizar</Btn>
        <Btn size="sm" variant="ghost" onClick={onTogglePause}>{paused ? '&#9654; Reanudar sync' : '&#9646;&#9646; Pausar sync'}</Btn>
      </div>
    </div>
  )
}
...
export function HNL(n) {
  return new Intl.NumberFormat('es-HN', { style:'currency', currency:'HNL', maximumFractionDigits:0 }).format(n || 0)
}

const sectionStyle  = { background:'#fff', borderRadius:'var(--radius)', border:'1px solid var(--border)', padding:'1.5rem', marginBottom:'1.5rem', boxShadow:'var(--shadow-sm)' }
const sectionHeader = { display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.25rem' }
const sectionTitle  = { fontSize:'1.2rem', color:'var(--ink)' }
const statCard      = { padding:'1.25rem 1.5rem', borderRadius:'var(--radius)', border:'1px solid', boxShadow:'var(--shadow-sm)' }
const statLabel     = { fontSize:'0.72rem', fontWeight:600, letterSpacing:'0.06em', textTransform:'uppercase', color:'var(--ink-muted)', marginBottom:'0.5rem' }
const statValue     = { fontSize:'1.9rem', fontWeight:700, lineHeight:1 }
const statusBar     = { display:'flex', alignItems:'center', flexWrap:'wrap', gap:'0.75rem', marginBottom:'1.5rem', padding:'0.75rem 1rem', background:'#fff', borderRadius:'var(--radius-sm)', border:'1px solid var(--border)', boxShadow:'var(--shadow-sm)' }
const statusLink    = { color:'var(--teal)', textDecoration:'none', fontSize:'0.85rem', fontWeight:500 }